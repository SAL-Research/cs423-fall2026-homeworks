# CS 423 — HW1 starter: Single-Cycle vs In-Order Pipeline
...
multi-arch-friendly: it builds/pulls natively on both x86-64 and Apple
Silicon machines. **Do not use any other gem5 version — results differ and
will not be regraded.**

## 2. Smoke test (do this before anything else — Task 1)

From this directory (the starter root):
...
the script prints `simInsts`, `simSeconds`, `hostSeconds`,
...
| `scripts/plot_results.py` | `summary.csv` → `plots/cpi.png`, `plots/speedup.png` |
